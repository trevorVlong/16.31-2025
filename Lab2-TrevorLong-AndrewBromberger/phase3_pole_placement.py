...
def requirements_to_poles(settling_time_req, overshoot_req):
    ...
    min_real_part = 4/settling_time_req
    ...
    margin_y = 1.3
    margin_z = 0.95
    pole_location = np.array([margin_x*min_real_part, margin_y*min_real_part, margin_z*min_real_part])
...
class PolePlacementController:
    ...
    def update(self, x_ref, y_ref, z_ref, x_meas, y_meas, z_meas):
        ...
        error = x - x_ref_vec  # REPLACE THIS
...
def test_pole_placement_response(tello, detector, controller, test_name):
    ...
    # Test positions (THESE ARE IN THE TAG FRAME!!!!!)
    x_initial, y_initial, z_initial = 0.0, 0.0, 1.5
    x_target, y_target, z_target = 0.3, 0.3, 2.0
...
def calculate_multi_axis_metrics(data):
    ...
    if all(s != float('inf') for s in settling_times):
        metrics['multi_axis_settling'] = np.max(settling_times)
        metrics['coordination_spread'] = np.max(settling_times) - np.min(settling_times)
    else:
